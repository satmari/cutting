		                ...
		                -->
			                    ...
			                    @foreach ($data as $req)
			                        <tr class="ss" id="item[]={{ $req->id }}" style="border-top: 3px solid grey;
										...
										<td>
											<a href="{{ url('paspul_prw2/'.$req->id) }}" class="btn btn-danger btn-xs center-block">
											Rewound paspul roll</a><br>
										<a href="{{ url('request_material_p/'.$req->paspul_roll_id) }}" class="btn btn-default btn-xs center-block">Material request</a>
										</td>
