    ...
    <div class="row">
        <div class="col-md-10 col-md-offset-1">
            <div class="panel panel-default">
                <div class="panel-heading">Cancel reservation material by PO</div>

                <div class="panel-body">
                    Item: <b>{{$input_item}}</b>, Variant: <b>{{$input_variant}}</b>, Batch: <b>{{$input_batch}}</b>
                </div>

                {!! Form::open(['method'=>'POST', 'url'=>'/cancel_po']) !!}
                    ...
                    <tbody class="searchable">
                     @foreach ($data as $req)
                        <tr>
                            ...
                            <td><big><b>{{ $req->po }}</b></big></td>
                            <td><span class="amount"> {{ floatval($req->qty) }}</span></td>
                            
                            <td>
                                    <input type="checkbox" id="box" class="btn box" name='checked[]' value="{{ $req->po }}">
                                    <input name="hidden[]" type='hidden' value="{{ $req->po }}"> 
                            </td>
