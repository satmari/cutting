...
//new prw2
	public function index2() {
		...
		// dd("Test");
		return view('prw.index2', compact('data','location','operators','operator'));
	}

	public function paspul_prw2($id) {
		// dd($id);
		...
		$data = DB::connection('sqlsrv')->select(DB::raw("SELECT 
			  ...
			  FROM [paspul_rewounds] as p1
			  WHERE status = 'TO_REWIND' AND p1.[id] = '".$id."' "));
			// dd($data);
		...
		return view('prw.paspul_prw2', compact('paspul_rewound_roll_id','paspul_rewound_roll','rewound_length_partialy','comment_office',
			'material','skeda','pasbin','dye_lot','color_desc','skeda_item_type','rewinding_method','priority',
			'call_shift_manager','comment_operator','comment_office','paspul_roll','paspul_roll_id'));
	}
	
	public function paspul_prw2_confirm(Request $request) {
		...
		if (($still_possible_to_plan == 0) AND ($last_roll == 'YES')) {
			...
			// save to paspul_rewound
				...
				$table_r->rewound_length_partialy = $rewound_length_partialy;
				$table_r->status = "TO_CUT";
				$table_r->position = $rewound_roll_position; //new
				$table_r->comment_operator = $comment_operator;
				$table_r->save();
			//
