...
class consController extends Controller {
	...
	public function update_cons_table() // Not using
	{
		//
			...
			for ($i=0; $i < count($data); $i++) { 
				...
				if (isset($info[0])) {
					...
					try {	
						...
						$table->status = $info[0]->{"po_status"};
						$table->cut_prod_line = $info[0]->{"po_flash"};
						$table->to_be_finished = $info[0]->{"po_fin"};
						...
						if (isset($info[1])) {
							$table->error = "Already consumed 2 or more extra materials";	
						}

						if (($table->over_cons - $table->extra_consumed) < 0) {
							$table->error = $table->error.", Consumed more than allowed overconsumption!";	
						}
	...
	{
		//
		$data = DB::connection('sqlsrv')->select(DB::raw("SELECT id,po,status FROM consumptions WHERE status = 'Released' "));
		// dd($data[0]->po);
		...
		if(isset($data[0])) {
			for ($i=0; $i < count($data); $i++) { 
				...
				$info = DB::connection('sqlsrv1')->select(DB::raw("SELECT				   
					   ...
					   (SELECT top 1 ile.[Item No_] FROM [Gordon_LIVE].[dbo].[GORDON\$Production Order] as po LEFT JOIN [Gordon_LIVE].[dbo].[GORDON\$Item Ledger Entry] as ile ON ile.[Simulated Order No_] = po.[No_]
						WHERE po.[Status] = 0 AND po.[No_] LIKE 'CUT%' AND po.[Docket No] LIKE '%wh%' AND ile.[Item No_] IS NOT NULL and po.[Shortcut Dimension 2 Code] LIKE '%".$data[$i]->po."%'   ) AS consumed_on_wh_item,

						(SELECT top 1 ile.[Variant Code] FROM [Gordon_LIVE].[dbo].[GORDON\$Production Order] as po LEFT JOIN [Gordon_LIVE].[dbo].[GORDON\$Item Ledger Entry] as ile ON ile.[Simulated Order No_] = po.[No_]
						WHERE po.[Status] = 0 AND po.[No_] LIKE 'CUT%' AND po.[Docket No] LIKE '%wh%' AND ile.[Item No_] IS NOT NULL and po.[Shortcut Dimension 2 Code] LIKE '%".$data[$i]->po."%'   ) AS consumed_on_wh_variant,
						
						(SELECT COUNT( DISTINCT  ile.[Item No_]) FROM [Gordon_LIVE].[dbo].[GORDON\$Production Order] as po LEFT JOIN [Gordon_LIVE].[dbo].[GORDON\$Item Ledger Entry] as ile ON ile.[Simulated Order No_] = po.[No_]
						WHERE po.[Status] = 0 AND po.[No_] LIKE 'CUT%' AND po.[Docket No] LIKE '%wh%' AND ile.[Item No_] IS NOT NULL and po.[Shortcut Dimension 2 Code] LIKE '%".$data[$i]->po."%'   ) AS consumed_on_wh_item_count,
					   
					   -SUM(ile.[Quantity]) AS consumed_tot,
					   (SELECT -SUM(ile.[Quantity]) FROM [Gordon_LIVE].[dbo].[GORDON\$Production Order] as po LEFT JOIN [Gordon_LIVE].[dbo].[GORDON\$Item Ledger Entry] as ile ON ile.[Simulated Order No_] = po.[No_]
						WHERE po.[Status] = 0 AND po.[No_] LIKE 'CUT%' AND po.[Docket No] LIKE '%wh%' AND ile.[Item No_] IS NOT NULL and po.[Shortcut Dimension 2 Code] LIKE '%".$data[$i]->po."%'   ) AS consumed_on_wh,
					   (SELECT case when [Status] = 3 then 'Released' else 'Check status' end FROM [Gordon_LIVE].[dbo].[GORDON\$Production Order] WHERE po.[Shortcut Dimension 2 Code] = [No_]) as po_status,
					   (SELECT [Cutting Prod_ Line] FROM [Gordon_LIVE].[dbo].[GORDON\$Production Order] WHERE po.[Shortcut Dimension 2 Code] = [No_]) as po_flash,
					   (SELECT case when [To be finished] = 1 then 'Yes' else 'No' end FROM [Gordon_LIVE].[dbo].[GORDON\$Production Order] WHERE po.[Shortcut Dimension 2 Code] = [No_]) as po_fin
				...
				LEFT JOIN [Gordon_LIVE].[dbo].[GORDON\$Item Ledger Entry] as ile ON ile.[Simulated Order No_] = po.[No_]
				WHERE 
					--po.[Status] = 0 AND po.[No_] LIKE 'CUT%' AND po.[Docket No] LIKE '%WH%' AND ile.[Item No_] IS NOT NULL AND 
					po.[Shortcut Dimension 2 Code] LIKE '%".$data[$i]->po."%'
				GROUP BY
					   --po.[Status],
					   --po.[No_],
					   --po.[Docket No],
				...
				order by po.[Shortcut Dimension 2 Code] desc
				"));
				// dd($info);
				...
				if (isset($info[0])) {
					...
					try {	
						...
						$table->extra_item = $info[0]->{"consumed_on_wh_item"};
						$table->extra_variant = $info[0]->{"consumed_on_wh_variant"};
						$table->extra_consumed = $info[0]->{"consumed_on_wh"};
						$table->extra_consumed_eur = NULL;

						$table->error = '';

						if ($info[0]->{"consumed_on_wh_item_count"} > 1) {
							$table->error = "Already consumed 2 or more extra materials ";	
						}
