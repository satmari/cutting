...
class pskController extends Controller {
	...
	public function index() {
		...
		if (Auth::check())
		{
			...
			return view('psk.error',compact('msg'));
		}
		// dd($operator1);
		...
		$count_ready_ki = DB::connection('sqlsrv')->select(DB::raw("SELECT COUNT([id]) as r 
			FROM [cutting].[dbo].[paspul_stocks] 
			WHERE location = 'READY_FOR_KIKINDA'  "));
		if (isset($count_ready_ki[0]->r)) {
			$count_ready_ki = $count_ready_ki[0]->r;
		} else {
			$count_ready_ki = 0;
		}

		$count_received_ki = DB::connection('sqlsrv')->select(DB::raw("SELECT COUNT([id]) as r 
			FROM [cutting].[dbo].[paspul_stocks] 
			WHERE location = 'RECEIVED_IN_KIKINDA'  "));
		if (isset($count_received_ki[0]->r)) {
			$count_received_ki = $count_received_ki[0]->r;
		} else {
			$count_received_ki = 0;
		}

		$count_kik = DB::connection('sqlsrv')->select(DB::raw("SELECT COUNT(s.[id]) as r
			FROM [cutting].[dbo].[paspul_stocks] as s
			INNER JOIN [cutting].[dbo].[paspul_locations] as l ON l.[location] = s.[location]
			WHERE l.[plant] = 'Kikinda' and s.[location] != 'RECEIVED_IN_KIKINDA'    "));
		if (isset($count_kik[0]->r)) {
			$count_kik = $count_kik[0]->r;
		} else {
			$count_kik = 0;
		}
		...
		return view('psk.index', compact('location','count_ready_ki','count_received_ki','count_kik','count_received_in_subotica'));
	}

	public function paspul_table_received_in_kik () {
		...
		"));
		// dd($data);
		return view('psk.table', compact('data'));
	}

	public function paspul_table_ready_for_kik () {
	...
	public function paspul_table_stock_ki () {
...
// LOC TO LOC 
	public function paspul_loc_to_loc_ki_from() {
		...
		WHERE l.[plant] = 'Kikinda'  "));

		return view('psk.paspul_loc_to_loc_ki_from', compact('from'));
	}

	public function paspul_loc_to_loc_ki_from_post(Request $request) {
		//
		...
		if (!empty($input['location1'])) {
			...
			return view('psk.paspul_loc_to_loc_ki_from', compact('from','msge'));
		}
		// dd($location_from);

		// check location
		$check_location = DB::connection('sqlsrv')->select(DB::raw("SELECT id FROM [cutting].[dbo].[paspul_locations] 
		WHERE [location] = '".$location_from."' and [plant] = 'Kikinda' "));
		...
		return view('psk.paspul_loc_to_loc_ki_pas', compact('location_from', 'pas_keys'));
	}

	public function paspul_loc_to_loc_ki_pas_post(Request $request) {
		//
		// $this->validate($request, ['po'=>'required']);
		$input = $request->all();
		// dd($input);
		...
		if (!empty($input['pas_one1'])) {
			...
			return view('psk.paspul_loc_to_loc_ki_pas', compact('location_from', 'pas_keys', 'msge'));
		}
		...
		return view('psk.paspul_loc_to_loc_ki_qty', compact('location_from', 'pas_one', 'current_qty'));
	}

	public function paspul_loc_to_loc_ki_qty_post (Request $request) {
		//
		// $this->validate($request, ['po'=>'required']);
		$input = $request->all();
		// dd($input);
		...
		if (($input['qty'] <= 0) OR ($input['qty'] > $current_qty)) {
			...
			return view('psk.paspul_loc_to_loc_ki_qty', compact('location_from', 'pas_one', 'current_qty', 'msge'));
		}

		$qty = (int)$input['qty'];
		// dd($qty);

		return view('psk.paspul_loc_to_loc_ki_op', compact('location_from', 'pas_one', 'qty'));
	}

	public function paspul_loc_to_loc_ki_op_post(Request $request) {
		//
		// $this->validate($request, ['po'=>'required']);
		$input = $request->all();
		// dd($input);
		...
		if (empty($input['op1'])) {
			...
			return view('psk.paspul_loc_to_loc_ki_op', compact('location_from', 'pas_one', 'qty', 'msge'));
		...
		WHERE [plant] = 'Kikinda' and type = 'stock' and [location] != '".$location_from."' "));

		return view('psk.paspul_loc_to_loc_ki_to', compact('location_from', 'pas_one', 'qty', 'op', 'to'));
	}

	public function paspul_loc_to_loc_ki_to_post (Request $request) {
		//
		// $this->validate($request, ['po'=>'required']);
		$input = $request->all();
		// dd($input);
		...
		$to =  DB::connection('sqlsrv')->select(DB::raw("SELECT [location] FROM [cutting].[dbo].[paspul_locations]
		WHERE [plant] = 'Kikinda' and type = 'stock' "));
		...
		if (!empty($input['location1'])) {
			...
			$msge = 'Please scan or select location';
			return view('psk.paspul_loc_to_loc_ki_to', compact('location_from', 'pas_one', 'qty', 'op', 'to', 'msge'));
		}
		// dd($location_to);

		// check location
		$check_location = DB::connection('sqlsrv')->select(DB::raw("SELECT id FROM [cutting].[dbo].[paspul_locations] 
		WHERE [location] = '".$location_to."' and [plant] = 'Kikinda' and [type] = 'stock' "));
...
// LOC TO PROD 
	public function paspul_loc_to_prod_ki_from() {

		$from =  DB::connection('sqlsrv')->select(DB::raw("SELECT DISTINCT s.[location] FROM [cutting].[dbo].[paspul_stocks] as s
		LEFT JOIN [cutting].[dbo].[paspul_locations] as l ON l.[location] = s.[location]
		WHERE l.[plant] = 'Kikinda'  "));

		return view('psk.paspul_loc_to_prod_ki_from', compact('from'));
	}

	public function paspul_loc_to_prod_ki_from_post(Request $request) {
		//
		// $this->validate($request, ['po'=>'required']);
		$input = $request->all(); // change use (delete or comment user Requestl; )
		// dd($input);
		...
		if (!empty($input['location1'])) {
			...
			$msge = 'Please scan or select location';
			return view('psk.paspul_loc_to_prod_ki_from', compact('from','msge'));
		}
		// dd($location_from);
		...
		return view('psk.paspul_loc_to_prod_ki_pas', compact('location_from', 'pas_keys'));
	}

	public function paspul_loc_to_prod_ki_pas_post(Request $request) {
		//
		// $this->validate($request, ['po'=>'required']);
		$input = $request->all();
		// dd($input);
		...
		if (!empty($input['pas_one1'])) {
			...
			$msge = 'Please scan or select paspul key';
			return view('psk.paspul_loc_to_prod_ki_pas', compact('location_from', 'pas_keys', 'msge'));
		}
		...
		return view('psk.paspul_loc_to_prod_ki_qty', compact('location_from', 'pas_one', 'current_qty'));
	}

	public function paspul_loc_to_prod_ki_qty_post (Request $request) {
		//
		// $this->validate($request, ['po'=>'required']);
		$input = $request->all();
		// dd($input);
		...
		if (($input['qty'] <= 0) OR ($input['qty'] > $current_qty)) {
			$msge = 'Please insert correct quantity';
			return view('psk.paspul_loc_to_prod_ki_qty', compact('location_from', 'pas_one', 'current_qty', 'msge'));
		}

		$qty = (int)$input['qty'];
		// dd($qty);

		return view('psk.paspul_loc_to_prod_ki_op', compact('location_from', 'pas_one', 'qty'));
	}

	public function paspul_loc_to_prod_ki_op_post(Request $request) {
		//
		// $this->validate($request, ['po'=>'required']);
		$input = $request->all();
		// dd($input);
		...
		if (empty($input['op1'])) {
			$msge = 'Please insert correct operator';
			return view('psk.paspul_loc_to_prod_ki_op', compact('location_from', 'pas_one', 'qty', 'msge'));
		...
		$to =  DB::connection('sqlsrv')->select(DB::raw("SELECT [location] FROM [cutting].[dbo].[paspul_locations]
		WHERE [plant] = 'Kikinda' and type = 'line' "));

		return view('psk.paspul_loc_to_prod_ki_to', compact('location_from', 'pas_one', 'qty', 'op', 'to'));
	}

	public function paspul_loc_to_prod_ki_to_post (Request $request) {
		//
		// $this->validate($request, ['po'=>'required']);
		$input = $request->all();
		// dd($input);
		...
		if (!empty($input['location1'])) {
			...
			return view('psk.paspul_loc_to_prod_ki_to', compact('location_from', 'pas_one', 'qty', 'op', 'to', 'msge'));
		}
		// dd($location_to);

		// check line
		$check_location = DB::connection('sqlsrv')->select(DB::raw("SELECT id,location FROM [cutting].[dbo].[paspul_locations] 
		WHERE [location] = '".$location_to."' and [plant] = 'Kikinda' and [type] = 'line' "));
		// dd($check_location);
...
// DELETE
	public function paspul_loc_to_del_ki_from() {

		$from =  DB::connection('sqlsrv')->select(DB::raw("SELECT DISTINCT s.[location] FROM [cutting].[dbo].[paspul_stocks] as s
		LEFT JOIN [cutting].[dbo].[paspul_locations] as l ON l.[location] = s.[location]
		WHERE l.[plant] = 'Kikinda'   "));

		return view('psk.paspul_loc_to_del_ki_from', compact('from'));
	}

	public function paspul_loc_to_del_ki_from_post(Request $request) {
		//
		// $this->validate($request, ['po'=>'required']);
		$input = $request->all(); // change use (delete or comment user Requestl; )
		// dd($input);
		...
		if (!empty($input['location1'])) {
			...
			$msge = 'Please scan or select location';
			return view('psk.paspul_loc_to_del_ki_from', compact('from','msge'));
		}
		// dd($location_from);
		...
		return view('psk.paspul_loc_to_del_ki_pas', compact('location_from', 'pas_keys'));
	}

	public function paspul_loc_to_del_ki_pas_post(Request $request) {
		//
		// $this->validate($request, ['po'=>'required']);
		$input = $request->all();
		// dd($input);
		...
		if (!empty($input['pas_one1'])) {
			...
			$msge = 'Please scan or select paspul key';
			return view('psk.paspul_loc_to_del_ki_pas', compact('location_from', 'pas_keys', 'msge'));
		}
		...
		return view('psk.paspul_loc_to_del_ki_qty', compact('location_from', 'pas_one', 'current_qty'));
	}

	public function paspul_loc_to_del_ki_qty_post (Request $request) {
		//
		// $this->validate($request, ['po'=>'required']);
		$input = $request->all();
		// dd($input);
		...
		if (($input['qty'] <= 0) OR ($input['qty'] > $current_qty)) {
			$msge = 'Please insert correct quantity';
			return view('psk.paspul_loc_to_del_ki_qty', compact('location_from', 'pas_one', 'current_qty', 'msge'));
		}

		$qty = (int)$input['qty'];
		// dd($qty);

		return view('psk.paspul_loc_to_del_ki_op', compact('location_from', 'pas_one', 'qty'));
	}

	public function paspul_loc_to_del_ki_op_post(Request $request) {
		//
		// $this->validate($request, ['po'=>'required']);
		$input = $request->all();
		// dd($input);
		...
		if (empty($input['op1'])) {
			$msge = 'Please insert correct operator';
			return view('psk.paspul_loc_to_del_ki_op', compact('location_from', 'pas_one', 'qty', 'msge'));
...
// RETURN
	public function paspul_ret_ki_to_su_from() {

		$from =  DB::connection('sqlsrv')->select(DB::raw("SELECT DISTINCT s.[location] FROM [cutting].[dbo].[paspul_stocks] as s
		LEFT JOIN [cutting].[dbo].[paspul_locations] as l ON l.[location] = s.[location]
		WHERE l.[plant] = 'Kikinda'  "));

		return view('psk.paspul_ret_ki_to_su_from', compact('from'));
	}

	public function paspul_ret_ki_to_su_from_post(Request $request) {
		//
		// $this->validate($request, ['po'=>'required']);
		$input = $request->all(); // change use (delete or comment user Requestl; )
		// dd($input);
		...
		if (!empty($input['location1'])) {
			...
			$msge = 'Please scan or select location';
			return view('psk.paspul_ret_ki_to_su_from', compact('from','msge'));
		}
		// dd($location_from);
		...
		return view('psk.paspul_ret_ki_to_su_pas', compact('location_from', 'pas_keys'));
	}

	public function paspul_ret_ki_to_su_pas_post(Request $request) {
		//
		// $this->validate($request, ['po'=>'required']);
		$input = $request->all();
		// dd($input);
		...
		if (!empty($input['pas_one1'])) {
			...
			$msge = 'Please scan or select paspul key';
			return view('psk.paspul_ret_ki_to_su_pas', compact('location_from', 'pas_keys', 'msge'));
		}
		...
		return view('psk.paspul_ret_ki_to_su_qty', compact('location_from', 'pas_one', 'current_qty'));
	}

	public function paspul_ret_ki_to_su_qty_post (Request $request) {
		//
		// $this->validate($request, ['po'=>'required']);
		$input = $request->all();
		// dd($input);
		...
		if (($input['qty'] <= 0) OR ($input['qty'] > $current_qty)) {
			$msge = 'Please insert correct quantity';
			return view('psk.paspul_ret_ki_to_su_qty', compact('location_from', 'pas_one', 'current_qty', 'msge'));
		}

		$qty = (int)$input['qty'];
		// dd($qty);

		return view('psk.paspul_ret_ki_to_su_op', compact('location_from', 'pas_one', 'qty'));
	}

	public function paspul_ret_ki_to_su_op_post(Request $request) {
		//
		// $this->validate($request, ['po'=>'required']);
		$input = $request->all();
		// dd($input);
		...
		if (empty($input['op1'])) {
			$msge = 'Please insert correct operator';
			return view('psk.paspul_ret_ki_to_su_op', compact('location_from', 'pas_one', 'qty', 'msge'));
